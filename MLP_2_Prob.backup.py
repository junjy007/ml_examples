...
def compute_image():
    for i, j in image:
        if i >= resolution:
            ...
            x1 = (float(j) / resolution) * (XBND) - (XBND)
            s = 0.0
            if wi_sele[None] >= 0:
                wi = wi_sele[None]
                w0 = W[wi_sele[None]][0]
...
while window.running:
    ...
    if window.is_pressed(ti.ui.LMB):
        if flag_select_w[None] and mouse[0] < 0.5:
            attemp_w[0][0] = (mouse[0] / 0.5) * (2*WBND) + (-WBND)
            attemp_w[0][1] = (mouse[1]) * (2*WBND) + (-WBND)
            select_w()

            # print("Try to find w at", attemp_w)
            # print("Found ", wi_sele[None], W[wi_sele[None]])
    ...
    canvas.circles(X_vis, radius=0.003, per_vertex_color=X_vis_clr)
    if wi_sele[None] >= 0:
